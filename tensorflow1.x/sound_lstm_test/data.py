...
def mel_batch_generator(batch_size=10, source=Source.DIGIT_WAVES):
	# maybe_download(source, DATA_DIR)
	batch_features = []
	files = os.listdir(path)
	...
	while True:
		...
		for file in files:
			...
			labels = [label] * 10
			for i in mel_split:
				i = np.pad(i, ((0, 0), (0, 80 - len(i[0]))), mode='constant',
									   constant_values=0)
				batch_features.append(np.array(i))
				if len(mel_split) >= batch_size:
...
def np_load(batch_size, batch_type, path='G:/sound_fixed/'):

	files = os.listdir(path + batch_type)
	labels_dict = one_hot_from_files()
	labels = []
	batchs = []
	...
	while True:
		...
		for file in files:
			label = labels_dict.get(file.split('_')[0])
			batch = np.load(path+batch_type+file)
			batchs.append(batch)
			labels.append(label)
			if len(batchs) >= batch_size:
				
				yield np.array(batchs), np.array(labels)
				batchs.clear()
				labels.clear()
